...
The Debian identity is lowercase `ttsroad`; the display name, launcher and installed directory stay
`TTSRoad`. `ttsroad.version` is the application SemVer and `ttsroad.debRevision` is only Debian's
revision for packaging-only rebuilds. Never fork the application version into packaging metadata.
The desktop file name generated by JDK 25 jpackage is `ttsroad-TTSRoad.desktop`; MPRIS's
`DesktopEntry` must match it without the `.desktop` suffix.
...
Debian tasks run `packaging/linux/finalize-deb.sh` after jpackage to add Section/Recommends and the
desktop fields its DSL cannot express. Do not bypass that finalizer when publishing an artifact.
